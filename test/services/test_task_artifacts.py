...
class TestTaskArtifacts(unittest.TestCase):
    ...
    def test_clean_expired_task_artifacts(self):
        """Expired task directories older than threshold should be handled safely."""
        old_dir = self.task_dir / "old_task"
        old_dir.mkdir()
        deleted = task_artifacts.clean_expired_task_artifacts(max_age_hours=0)

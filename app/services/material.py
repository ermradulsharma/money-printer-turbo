...
def generate_videos_wavespeed(
    ...
    if not api_key:
        logger.error("wavespeed_api_keys is not configured; skipping WaveSpeed generation")
        return []
    ...
    if prompt and len(prompt) < 15 and not prompt.lower().startswith("cinematic"):
        prompt = f"cinematic high quality video footage of {prompt}, 4k resolution"
...
def _wait_for_wavespeed_prediction(
    *,
    ...
    while True:
        ...
        adaptive_delay = min(WAVESPEED_POLL_INTERVAL_SECONDS + (poll_count // 3), 5)
...
def download_videos(
    ...
    # Dynamic Provider Fallback Chain for stock media searching
    provider_fallback_chain = []
    if source == "pexels":
        provider_fallback_chain = [("pexels", search_videos_pexels), ("pixabay", search_videos_pixabay), ("coverr", search_videos_coverr)]
    elif source == "pixabay":
        provider_fallback_chain = [("pixabay", search_videos_pixabay), ("pexels", search_videos_pexels), ("coverr", search_videos_coverr)]
    elif source == "coverr":
        provider_fallback_chain = [("coverr", search_videos_coverr), ("pexels", search_videos_pexels), ("pixabay", search_videos_pixabay)]
    ...
    def search_videos(
        ...
        for prov_name, search_func in provider_fallback_chain:
            try:
                ...
                logger.warning(
                    f"stock provider search failed, attempting fallback: "
                    f"provider={prov_name}, error={type(exc).__name__}, detail={exc}"
                )
        return []
